defmodule BrokerageWeb.AlpacaController do
  @moduledoc """
  ...
  alias Brokerage.Alpaca
  alias Brokerage.BrokerageAccounts
  alias Brokerage.BrokerFunding.Client, as: BrokerClient
  ...
  def news(conn, %{"symbol" => symbol}) when is_binary(symbol) do
    ...
    # Brokerage.News for the merge / dedupe logic; falls back
    # to whichever provider is available when one is unconfigured.
    render_result(conn, Brokerage.News.for_symbol(symbol))
  end

  def news(conn, _), do: bad_request(conn, "missing symbol")

  # Company profile — aggregated by Brokerage.CompanyProfile.
  ...
  def profile(conn, %{"symbol" => symbol}) when is_binary(symbol) do
    ...
    profile =
      case Cachex.get(:alpaca_cache, cache_key) do
        ...
        _ ->
          {:ok, normalized} = Brokerage.CompanyProfile.for_symbol(sym)
          ...
          {:ok, account_id} ->
            ...
            case result do
              {:ok, order} when is_map(order) ->
                Brokerage.Notifications.order_placed(
