defmodule Brokerage.BrokerFunding do
  @moduledoc """
  ...
  alias Brokerage.BrokerFunding.{BrokerApi, Client, Deposit}
  alias Brokerage.{BrokerageAccounts, WishlistItems}
        ...
        case Client.create_transfer(account.alpaca_account_id, body) do
          {:ok, %{"id" => transfer_id} = resp} ->
            ...
            if deposit.direction == "OUTGOING" do
              Brokerage.Notifications.withdrawal_initiated(deposit.user_id, deposit)
            end
  ...
  def handle_webhook(payload) when is_map(payload) do
    ...
    with id when is_binary(id) <- transfer["id"],
      ...
      if updated.status == "completed" and deposit.status != "completed" do
        ...
        if Brokerage.AlpacaMock.Server.enabled?() do
          Brokerage.AlpacaMock.Server.complete_transfer(id)
        end
        ...
        if updated.direction != "OUTGOING" do
          Brokerage.Notifications.deposit_settled(updated.user_id, updated)
        end
  ...
  defp place_wishlist_order(account_id, item) do
    ...
    case Client.place_order(account_id, body) do
      {:ok, %{"id" => order_id}} ->
        ...
        Brokerage.Notifications.wishlist_filled(item.user_id, item)
        {:ok, item}
