defmodule Brokerage.BrokerageAccounts do
  @moduledoc """
  ...
  *their* Alpaca account, not the shared `B2B_TEST_ACCOUNT_ID`). In
  ...
  alias Brokerage.BrokerFunding.{BrokerApi, Client}
  alias Brokerage.BrokerageAccounts.{Account, AchRelationship}
  ...
  defp update_kyc_state(%Account{} = a, state) do
    ...
    if state == "active" and prior != "active" do
      case result do
        {:ok, updated} ->
          Brokerage.Notifications.kyc_approved(updated.user_id, updated)
